...
function loadJdPrice(){
	var url="http://paimai.jd.com/json/current/queryJdPrice?sku="+jd_globe.sku+"&paimaiId="+jd_globe.paimaiId;
...
function bidWithCurrentPrice(price){
	...
	//根据包邮价进行设定
	if(bid_price >= jd_globe.mail_free-5 && bid_price < jd_globe.mail_free) bid_price = jd_globe.mail_free;
